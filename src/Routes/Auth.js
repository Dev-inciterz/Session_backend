...
// New route custom expiration time
router.post("/update-token-custom-expiry", Auth, async (req, res) => {

console.log("wgchjkbverhjkvrv")
    const { expirationDays } = req.body;

    console.log("tyhe body", expirationDays)


    console.log("tyhe user", req.user)
    ...
    try {
      ...
      user.token = token;
      user.tokenExpire = Date.now() + expirationDays * 24 * 60 * 60 * 1000;
